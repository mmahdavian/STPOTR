...
#      decoder_outputs_euler[i] = euler.reshape((tgt_seq_len, -1))
  ...
  def load_data(self):
    ...
    all_traj=[]
    all_traj_=[]
    my_data = np.load('/home/mohammad/Mohammad_ws/future_pose_prediction/potr/data/data_3d_h36m.npz', allow_pickle=True)['positions_3d'].item()
 ...
 #       n_frames, dims = action_sequence.shape
    ...
    #    action_sequence = action_sequence[even_idx, :]
        ...
        for action in sorted(self.my_keys):
            correct_action = sorted(self._action_defs)[int(i/2)]
            sact = i%2+1
