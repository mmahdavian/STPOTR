...
class H36MDataset(torch.utils.data.Dataset):
  ...
  def __init__(self, 
    ...
    self.js_left = [3, 4, 5, 10, 11, 14]
    self.js_right = [0, 1, 2, 13, 14, 15]
    ...
    self._all_data,self._all_trajectory = self.load_data()
...
#    data_sel = self.convert_to_format(data_sel)
  ...
  def _get_item_train(self):
    ...
    decoder_inputs_traj[:, 0:input_size] = \
      data_sel_traj[src_seq_len:src_seq_len + target_seq_len, :]
    # source_seq_len = src_seq_len + 1
    ...
    for s_id in self._data_ids:
        ...
        for action in sorted(self.my_keys):
            ...
            for cam in anim['cameras']:
                correct_action = sorted(self._action_defs)[int(i/16)]
                sact = i%16+1
                ...
                action_sequence_flip = np.copy(action_sequence)
                ## for flipping data
                for j in range(2):
                    ...
                    if j==1:
                        nf,nj,dim = action_sequence_flip.shape
                        action_sequence = action_sequence_flip
                        action_sequence[:,:,0] *= -1
                        action_sequence[:, self.js_left + self.js_right] = action_sequence[i, :, self.js_right + self.js_left]
                        traj_sequence[:,0] *= -1
