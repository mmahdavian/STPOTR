...
class PoseTransformer(nn.Module):
  ...
  def __init__(self,
    ...
    self._traj_embedding = traj_embedding
    self._traject_decoder = traj_decoder
    ...
    self._include_last_obs = include_last_obs
    
    self._transformer = Transformer(
        num_encoder_layers=num_encoder_layers,
        num_decoder_layers=num_decoder_layers,
        model_dim=model_dim,
        model_dim_traj=model_dim_traj,
        num_heads=num_heads,
        dim_ffn=dim_ffn,
        dropout=dropout,
        init_fn=init_fn,
    ...
    self._traj_decoder = PositionEncodings.PositionEncodings1D(
        num_pos_feats=self._model_dim_traj,
        temperature=self._pos_encoding_params[0],
        alpha=self._pos_encoding_params[1]
    )
    ...
    if self._predict_activity:
      if self._include_last_obs:
          self._action_head_size = self._model_dim if self._use_class_token \
              else self._model_dim*(self._source_seq_length)
      else:
          self._action_head_size = self._model_dim if self._use_class_token \
  ...
  def init_query_embedding(self):
    ...
    self._query_embed_traj = nn.Embedding(self._target_seq_length, self._model_dim_traj)
    print('[INFO] ({}) Init query embeddings!'.format(self.__class__.__name__))
    nn.init.xavier_uniform_(self._query_embed.weight.data)
    nn.init.xavier_uniform_(self._query_embed_traj.weight.data)
  ...
  def init_position_encodings(self):
    ...
    decoder_traj_encodings = self._traj_decoder(self._target_seq_length).view(
            self._target_seq_length, 1, self._model_dim_traj)
    ...
    self._decoder_traj_encodings = nn.Parameter(
        decoder_traj_encodings, requires_grad=False)
  ...
  def forward(self, 
    ...
    Args:
      ...
      input_traj_seq: Shape [batch_size, src_sequence_length, 3].
      target_traj_seq: Shape [batch_size, tgt_sequence_length, 3].
  ...
  def forward_training(self,
    ...
    input_traj_seq = input_traj_seq_
    target_traj_seq = target_traj_seq_
    ...
    if self._pose_embedding is not None:
      ...
      input_traj_seq = self._traj_embedding(input_traj_seq)
      target_traj_seq = self._traj_embedding(target_traj_seq)
    ...
    input_traj_seq = torch.transpose(input_traj_seq, 0, 1)
    target_traj_seq = torch.transpose(target_traj_seq, 0, 1)
    ...
    attn_output, memory, attn_weights, enc_weights, mat, attn_output_traj, memory_traj = self._transformer(
        input_pose_seq,
        target_pose_seq,
        input_traj_seq,
        target_traj_seq,
        query_embedding = self._query_embed.weight,
        query_embedding_traj = self._query_embed_traj.weight,
        encoder_position_encodings=self._encoder_pos_encodings,
        decoder_position_encodings=self._decoder_pos_encodings,
        encoder_trajectory_encodings=self._encoder_traj_encodings,
        decoder_trajectory_encodings=self._decoder_traj_encodings,
    ...
    for l in range(self._num_decoder_layers):
      ...
      ###
      in_out_concat = torch.cat((input_pose_seq,self._pose_embedding(out_sequence_)),dim=0)
      in_out_concat = in_out_concat + self._encoder_decoder_pos_encodings
      qkv = self.qkv(in_out_concat)
      q,k,v = qkv.chunk(3,dim=-1)
      
      end_attn_output, end_attn_output_weights = self._self_attn(q,k,v,need_weights=True)
      end_sequence_ = self._pose_decoder(
          end_attn_output.view(-1, self._model_dim))
      end_sequence_ = end_sequence_.view(
            self.src_len+self._target_seq_length, -1, self._pose_dim)
      end_sequence_[:self._source_seq_length] = input_pose_seq_.permute(1,0,2)
      
      if mode=='train':
          mask_idx = sorted(random.sample(range(self._target_seq_length),k=int(self._gt_ratio*self._target_seq_length)))
          mask = torch.zeros(end_sequence_[self._source_seq_length:].permute(1,0,2).shape)
          mask[:,mask_idx,:]=1
          mask = mask.cuda()
          out_sequence_ = end_sequence_[self._source_seq_length:].permute(1,0,2)*(1-mask).cuda() + gt_pose_seq_*mask.cuda()      

      elif mode=='eval':
          mask_idx = sorted(random.sample(range(self._target_seq_length),k=0))  
          mask = torch.zeros(end_sequence_[self._source_seq_length:].permute(1,0,2).shape)
          mask[:,mask_idx,:]=1
          mask = mask
          out_sequence_ = end_sequence_[self._source_seq_length:].permute(1,0,2)     
...
#      out_sequence_ = torch.transpose(out_sequence_, 0, 1)
      ...
      # [target_seq_length, batch_size, pose_dim_traj]
      out_sequence_traj_ = out_sequence_traj_.view(
          self._target_seq_length, -1, 3)
      # apply residual connection between target query and predicted pose
...
def model_factory(params, pose_embedding_fn, pose_decoder_fn, traj_embedding_fn, traj_decoder_fn):
  ...
  return PoseTransformer(
      ...
      model_dim_traj=params['model_dim_traj'],
      ...
      traj_embedding=traj_embedding_fn(params),
